...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    //Recursive Cases
    //First case value to add greater than value in current
    if (compare(val, cur->val) == -1)
    {
        cur->left = _addNode(cur->left, val);
    }
    //Second case if current value greater than value to add
...
{
	struct Node *nav = tree->root;

	while (nav != 0)
    {
        //If equal, return true
        if (compare(nav->val, val) == 0)
        {
            return 1;
        }
        //If passed value smaller than current, go to left node
        else if (compare(val, nav->val) == -1)
        {
            nav = nav->left;
        }
        else
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    {
        //If cur->right is null nothing larger than cur
        if(cur->right == 0)
        {
            nav = cur->left;
            free(cur);
        ...
        //Otherwise, get the leftmost element of right subtree
        cur->val = _leftMost(cur->right);
        cur->right = _removeLeftMost(cur->right);
    }
    //Otherwise, call recursive cases on right or left subtree
    else if (compare(val, cur->val) == -1)
    {
