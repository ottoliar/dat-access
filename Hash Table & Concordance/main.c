...
int main (int argc, const char * argv[]) {
    ...
    /*... concordance code goes here ...*/
        ...
        //If file couldn't be located, print error msg
        if (!fileptr)
        {
            printf("Failed to open input file.\n");
        }

        //While end of file not reaching, take in a word
        while (!feof(fileptr))
        {
            ...
            if (word)
            {
            	//If the key is already in table, increment the count
                ...
                //Otherwise, insert new key into table
                {
	...
	printMap(hashTable);
	timer = clock() - timer;
	printf("\nusing hashing function: %d", HASHING_FUNCTION);
