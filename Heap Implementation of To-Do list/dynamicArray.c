...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	...
	assert(j < sizeDynArr(heap));
  	...
  	if (compare(getDynArr(heap, i), getDynArr(heap, j)) <= 0) //i has lower priority
  		return i;
...
void addHeap(DynArr *heap, TYPE val)
{
	...
	//Perlocate up the heap
	while (pos != 0)
	{
		parent = (pos-1)/2; //parent node of the recently added node

		//If the parent has greater priority than pos, swap the elements
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	//Find indices of the child nodes
	int leftIdx = (pos * 2) + 1;
	int rightIdx = (pos * 2) + 2;
	int smallestChild;

	if (rightIdx < max) //Check to see if there is two children
	{
		//Find the smaller (higher priority) of the two child nodes
		smallestChild = _smallerIndexHeap(heap, leftIdx, rightIdx);

		//Swap the smallest child and the pos if pos has higher priority
		if (compare(getDynArr(heap, smallestChild), getDynArr(heap, pos)) == -1)
		{
	...
	else if (leftIdx <= max) //Only one child
	{
		//If pos has higher priority, swap with leftIdx
...
{
   assert(!isEmptyDynArr(heap));
   int last = sizeDynArr(heap) - 1; //index of last element

   //Put the last element of the heap at root
   putDynArr(heap, 0, getDynArr(heap, last));

   //Remove the old root
   removeAtDynArr(heap, last);

   //Perlocate down, rebuilding the heap up to but not including last
...
void _buildHeap(DynArr *heap)
{
	...
	int start;
	//Start at root node of first subtree not guaranteed to be proper heap
	for (start = (sizeDynArr(heap) / 2) - 1; start >= 0; start--)
	{
		_adjustHeap(heap, sizeDynArr(heap), start);
	}
}
/*
...
void sortHeap(DynArr *heap)
{
	...
	//Build the dynamic array so that it conforms to heap properties
	_buildHeap(heap);

	int end;
	for (end = sizeDynArr(heap) - 1; end > 0; end--)
	{
		//Swap first and last elements
		swapDynArr(heap, 0, end);

		//Adjust up to, but not including, last element
		_adjustHeap(heap, end - 1, 0);
	}
