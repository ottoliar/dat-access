...
int main(int argc, char* argv[]){
	...
	printf("Testing size...\n");
	assertTrue(EQ(sizeDynArr(dyn), 3), "Size should be 3");
	printf("Removing at position 2 (index 1 in array)...\n");
	removeAtDynArr(dyn, 1);
	printf("After Remove--Element at index 0: %lf\n", getDynArr(dyn, 0));
	printf("After Remove--Element at index 1 (Was 5 before remove): %lf\n", getDynArr(dyn, 1));
	assertTrue(EQ(sizeDynArr(dyn), 2), "Size should now be 2");
